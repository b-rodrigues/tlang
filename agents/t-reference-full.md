...
- Constructors and I/O: `dataframe(...)`, `read_csv(path, separator = ",", skip_header = false, skip_lines = 0, clean_colnames = false)`, `read_parquet(path)`, `read_ipc(path)`, `write_csv(df, path)`, `write_ipc(df, path)`
