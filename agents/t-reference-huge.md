...
T can export a pipeline or meta-pipeline as a [Mermaid](https://mermaid.js.org/) flowchart via `pipeline_to_mermaid()`:
...
### Node Colors by Runtime

Each node is shaded according to its language runtime, making it easy to distinguish polyglot pipelines at a glance:
...
Nodes that encountered an error during execution keep their runtime fill color but get a red border (`stroke:#ff0000, stroke-width:3px`), while healthy nodes use a subtle gray border.

```t
-- Example output with mixed runtimes:
--   graph LR
--     clean["clean [R]"];
--     model["model [Python]"];
--     report["report [Quarto]"];
--     clean --> model;
--     model --> report;
--     style clean fill:#246ABF,color:#000000,stroke:#333,stroke-width:1px
--     style model fill:#FFD343,color:#000000,stroke:#ff0000,stroke-width:3px
--     style report fill:#4F789E,color:#000000,stroke:#333,stroke-width:1px
```

### Viewing Mermaid Graphs in the Browser

You can pass a pipeline, meta-pipeline, or a raw Mermaid string directly to `show_plot()`. T generates a temporary HTML file with the Mermaid JS engine and opens it in your browser:

```t
-- Visualize a pipeline:
show_plot(p)

-- Visualize a raw Mermaid string:
show_plot("graph TD\n  Start --> Stop")
```

This works without any Nix build — the diagram is rendered client-side in the browser.

See the [Pipeline Tutorial](pipeline_tutorial.html) for more details on `pipeline_to_mermaid` and interactive pipeline visualization.
