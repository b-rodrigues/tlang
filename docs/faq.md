...
- **Native Escape Hatch**: If you need to manipulate a complex object that cannot be serialized, R/Python/Julia runtime nodes expose a native `read_node()` helper. By default it uses the runtime's standard loader (`readRDS`, Python pickle deserialization, or Julia `Serialization.deserialize`), but it can also return the artifact path directly (`path = TRUE` / `path=True` / `path=true`) or use a custom deserializer.
