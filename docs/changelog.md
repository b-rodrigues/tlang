...
- **Debug Subshell Package Manager Guards**: All three language debug subshells now actively block imperative package installation commands. Running `pip install`, `Pkg.add()`, `install.packages()`, or equivalent tools inside a debug REPL raises an immediate error with a message directing you to declare the package in `tproject.toml` instead, preserving reproducibility.
