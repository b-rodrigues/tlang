...
- **`t fix` suggests renaming colliding node names**: the reserved-name error now carries a `Rename_node` suggested fix that appends `_node` to the colliding name (`count` → `count_node`). `t fix` / `t_fix()` applies it mechanically to the node's definition line without touching top-level assignments.
