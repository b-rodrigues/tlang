...
- **Stabilized Pipeline Dependency Detection**: Refactored the lexical analyzer to prevent false-positive dependencies in polyglot pipelines.
    - **Strict Comment Stripping**: The pipeline resolver now automatically strips lines starting with `--` or `#` within foreign code blocks (`<{ ... }>`) before extraction. This prevents node names mentioned in comments from being incorrectly identified as dependencies.
    - **Explicit `deps` Argument**: Replaced legacy comment-based annotations with a first-class `deps` argument in node definitions (`node`, `rn`, `pyn`, `shn`). This allows for robust, explicit dependency declaration using bare identifiers (e.g., `deps = [raw_data, summary_py]`), bypassing automatic lexical inference when necessary.
    - **No Backward Compatibility**: Removed support for the legacy `--# @deps` comment syntax to ensure a single, unambiguous way to declare manual dependencies.
