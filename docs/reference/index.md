# Function Reference

| Function | Description |
| --- | --- |
| [abs](abs.md) | Absolute value |
| [acos](acos.md) | Inverse cosine |
| [acosh](acosh.md) | Inverse hyperbolic cosine |
| [add_diagnostics](add_diagnostics.md) | Add Model Diagnostics |
| [apropos](apropos.md) | Search for functions by keyword |
| [args](args.md) | Get function arguments and their types |
| [arrange](arrange.md) | Arrange rows |
| [asin](asin.md) | Inverse sine |
| [asinh](asinh.md) | Inverse hyperbolic sine |
| [assert](assert.md) | Assert Condition |
| [atan](atan.md) | Inverse tangent |
| [atan2](atan2.md) | Two-argument arctangent |
| [atanh](atanh.md) | Inverse hyperbolic tangent |
| [body](body.md) | Get function body |
| [build_pipeline](build_pipeline.md) | Build Pipeline Artifacts |
| [build_pipeline_internal](build_pipeline_internal.md) | Build Pipeline Internally |
| [case_when](case_when.md) | Vectorized Case-When |
| [casewhen](casewhen.md) | Vectorized case-when |
| [cat](cat.md) | Print values without escaping |
| [cbind](cbind.md) | Column bind matrices |
| [ceil](ceil.md) | Ceiling alias |
| [ceiling](ceiling.md) | Ceiling function |
| [char_at](char_at.md) | Get character at index |
| [clean_colnames](clean_colnames.md) | Clean DataFrame Column Names |
| [clean_names](clean_names.md) | Clean Column Names |
| [colnames](colnames.md) | Get column names |
| [contains](contains.md) | Check if string contains substring |
| [cor](cor.md) | Correlation |
| [cos](cos.md) | Cosine |
| [cosh](cosh.md) | Hyperbolic cosine |
| [cov](cov.md) | Covariance |
| [cumall](cumall.md) | Cumulative All |
| [cumany](cumany.md) | Cumulative Any |
| [cume_dist](cume_dist.md) | Cumulative Distribution |
| [cummax](cummax.md) | Cumulative Maximum |
| [cummean](cummean.md) | Cumulative Mean |
| [cummin](cummin.md) | Cumulative Minimum |
| [cumsum](cumsum.md) | Cumulative Sum |
| [cv](cv.md) | Coefficient of variation |
| [dataframe](dataframe.md) | Create a DataFrame |
| [dense_rank](dense_rank.md) | Dense Rank |
| [deserialize](deserialize.md) | Deserialize Value |
| [deserialize_from_file](deserialize_from_file.md) | Binary Deserialization |
| [diag](diag.md) | Create or extract diagonal |
| [ends_with](ends_with.md) | Check if string ends with suffix |
| [error](error.md) | Raise Error |
| [error_code](error_code.md) | Get error code |
| [error_context](error_context.md) | Get error context |
| [error_message](error_message.md) | Get error message |
| [exp](exp.md) | Exponential function |
| [explain](explain.md) | Explain Value |
| [explain_json](explain_json.md) | Explain Value as JSON |
| [expr](expr.md) | Capture an expression |
| [exprs](exprs.md) | Capture multiple expressions |
| [filter](filter.md) | Filter rows |
| [fit_stats](fit_stats.md) | Model Fit Statistics |
| [fivenum](fivenum.md) | Five-number summary |
| [floor](floor.md) | Floor function |
| [get](get.md) | Get element by index |
| [glimpse](glimpse.md) | Glimpse DataFrame |
| [greet](greet.md) | Greet someone |
| [group_by](group_by.md) | Group by columns |
| [head](head.md) | Get the first n rows/items |
| [help](help.md) | Display documentation for a function |
| [huber_loss](huber_loss.md) | Huber loss |
| [ifelse](ifelse.md) | Vectorized If-Else |
| [index_of](index_of.md) | Find index of substring |
| [inspect_node](inspect_node.md) | Inspect Pipeline Node Metadata |
| [inspect_pipeline](inspect_pipeline.md) | Inspect Pipeline Logs |
| [intent_fields](intent_fields.md) | Get All Intent Fields |
| [intent_get](intent_get.md) | Get Intent Field |
| [inv](inv.md) | Matrix inverse |
| [iota](iota.md) | Create a vector of ones |
| [iqr](iqr.md) | Interquartile range |
| [is_empty](is_empty.md) | Check if string is empty |
| [is_error](is_error.md) | Check if a value is an Error |
| [is_na](is_na.md) | Check for NA |
| [join](join.md) | Join strings with a separator |
| [json_escape](json_escape.md) | JSON String Escaper |
| [json_unescape](json_unescape.md) | JSON String Unescaper |
| [kron](kron.md) | Kronecker product |
| [kurtosis](kurtosis.md) | Excess kurtosis |
| [lag](lag.md) | Lag values |
| [last_index_of](last_index_of.md) | Find last index of substring |
| [lead](lead.md) | Lead values |
| [length](length.md) | Get length |
| [list_logs](list_logs.md) | List Pipeline Logs |
| [lm](lm.md) | Linear Model |
| [log](log.md) | Natural logarithm |
| [mad](mad.md) | Median absolute deviation |
| [map](map.md) | Map a function over a list |
| [matmul](matmul.md) | Matrix multiplication |
| [max](max.md) | Maximum value |
| [mean](mean.md) | Compute arithmetic mean of numeric values |
| [median](median.md) | Median |
| [min](min.md) | Minimum value |
| [min_rank](min_rank.md) | Minimum Rank |
| [mode](mode.md) | Mode |
| [mutate](mutate.md) | Create or modify columns |
| [na](na.md) | Generic NA |
| [na_bool](na_bool.md) | Boolean NA |
| [na_float](na_float.md) | Float NA |
| [na_int](na_int.md) | Integer NA |
| [na_string](na_string.md) | String NA |
| [nchar](nchar.md) | Get character count |
| [ncol](ncol.md) | Number of columns |
| [ndarray](ndarray.md) | Create an N-dimensional array |
| [ndarray_data](ndarray_data.md) | Get NDArray data |
| [node](node.md) | Configure a Pipeline Node |
| [normalize](normalize.md) | Normalize values |
| [nrow](nrow.md) | Number of rows |
| [ntile](ntile.md) | N-tiles |
| [package_info](package_info.md) | Get package information |
| [packages](packages.md) | List available packages |
| [parse_file](parse_file.md) | Parse T-Doc Comments |
| [percent_rank](percent_rank.md) | Percent Rank |
| [pipeline_deps](pipeline_deps.md) | List Node Dependencies |
| [pipeline_node](pipeline_node.md) | Get Pipeline Node |
| [pipeline_nodes](pipeline_nodes.md) | List Pipeline Nodes |
| [pipeline_run](pipeline_run.md) | Run Pipeline |
| [populate_pipeline](populate_pipeline.md) | Populate Pipeline |
| [pow](pow.md) | Power function |
| [pretty_print](pretty_print.md) | Pretty-print a value |
| [print](print.md) | Print values to standard output |
| [pull](pull.md) | Extract column as vector |
| [quantile](quantile.md) | Quantiles |
| [range](range.md) | Range |
| [read_csv](read_csv.md) | Read CSV file |
| [read_node](read_node.md) | Read Pipeline Node Artifact |
| [read_registry](read_registry.md) | Registry Maintenance (Reader) |
| [rebuild_node](rebuild_node.md) | Rebuild a Pipeline Node |
| [replace](replace.md) | Replace all occurrences |
| [replace_first](replace_first.md) | Replace first occurrence |
| [reshape](reshape.md) | Reshape an NDArray |
| [round](round.md) | Round values |
| [row_number](row_number.md) | Row Number |
| [run_doctor](run_doctor.md) | Run Package/Project Doctor |
| [scaffold_package](scaffold_package.md) | Scaffold a new T package |
| [scaffold_project](scaffold_project.md) | Scaffold a new T project |
| [scale](scale.md) | Scale values |
| [sd](sd.md) | Standard Deviation |
| [select](select.md) | Select columns |
| [seq](seq.md) | Generate a sequence of integers |
| [serialize](serialize.md) | Serialize Value |
| [serialize_to_file](serialize_to_file.md) | Binary Serialization |
| [shape](shape.md) | Get NDArray dimensions |
| [sign](sign.md) | Sign of number |
| [signif](signif.md) | Significant-digit rounding |
| [sin](sin.md) | Sine |
| [sinh](sinh.md) | Hyperbolic sine |
| [skewness](skewness.md) | Skewness |
| [slice](slice.md) | Extract slice |
| [source](source.md) | Get function source code |
| [sprintf](sprintf.md) | Format a string |
| [sqrt](sqrt.md) | Square root |
| [standardize](standardize.md) | Standardize values |
| [starts_with](starts_with.md) | Check if string starts with prefix |
| [string](string.md) | Convert to string |
| [substring](substring.md) | Extract substring |
| [sum](sum.md) | Sum of numeric values |
| [summarize](summarize.md) | Summarize data |
| [summary](summary.md) | Model Summary |
| [t_doc](t_doc.md) | Generate Documentation |
| [t_read_json](t_read_json.md) | Read Value from JSON |
| [t_run](t_run.md) | Run a T script |
| [t_test](t_test.md) | Run tests |
| [t_write_json](t_write_json.md) | Write Value to JSON |
| [tail](tail.md) | Get the last n rows/items |
| [tan](tan.md) | Tangent |
| [tanh](tanh.md) | Hyperbolic tangent |
| [to_array](to_array.md) | Convert to NDArray |
| [to_lower](to_lower.md) | Convert to lowercase |
| [to_upper](to_upper.md) | Convert to uppercase |
| [trace_nodes](trace_nodes.md) | Trace Pipeline Nodes |
| [transpose](transpose.md) | Transpose matrix |
| [trimmed_mean](trimmed_mean.md) | Trimmed mean |
| [trunc](trunc.md) | Truncate values |
| [type](type.md) | Get the type name of a value |
| [ungroup](ungroup.md) | Remove grouping |
| [update_flake_lock](update_flake_lock.md) | Update Dependencies |
| [var](var.md) | Variance |
| [winsorize](winsorize.md) | Winsorize values |
| [write_csv](write_csv.md) | Write CSV file |
| [write_registry](write_registry.md) | Registry Maintenance (Writer) |
| [write_text](write_text.md) | Write text to a file |
