# Specification: Julia Plot Support for `make.jl`, `TidierPlots.jl`, and `Plots.jl`
...
- let `show_plot("sales_plot")` render a PNG
...
- `class` — one-line file containing `tidierplots` or `plotsjl`
...
- Julia visual-class resolution that returns `tidierplots` or `plotsjl`
...
`TidierPlots.jl` should be treated as the Julia analogue of `ggplot`/`plotnine`, while `Plots.jl` should be treated as the Julia analogue of `matplotlib`: useful metadata, explicit rendering, and no hidden fallbacks.
