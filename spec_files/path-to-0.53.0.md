...
- [x] **Execution Plan Serialization**: Allow `dry_run = true` to return a structured DataFrame with columns `node`, `action` (`rebuild` | `cache_hit`), and `store_path` to enable programmatic execution plan analysis and abort thresholds.
