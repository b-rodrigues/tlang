...
let run_tests pass_count fail_count _eval_string _eval_string_env _test =
  ...
  (match Arrow_column.get_column tbl_10k "value" with
   | Some col_view ->
     ...
     (match v with
      | Ast.VNull ->
        incr pass_count; Printf.printf "  success get_value_at out-of-bounds returns VNull\n"
      | _ ->
  ...
  (match Arrow_compute.compare_column_scalar cmp_tbl "x" 5.0 "invalid" with
   | None ->
     incr pass_count; Printf.printf "  success compare_column_scalar returns None for invalid op\n"
   | Some _ ->
     incr fail_count; Printf.printf "  failure compare_column_scalar should return None for invalid op\n");
  print_newline ();
  ...
  incr pass_count; Printf.printf "  success Sum 100k rows (%.4fs)\n" t_sum100k;
  ...
  incr pass_count; Printf.printf "  success Sum 1M rows (%.4fs)\n" t_sum1m;
  ...
  incr pass_count; Printf.printf "  success Group aggregate mean 1M rows (%.4fs)\n" t_gagg1m;
