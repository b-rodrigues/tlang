...
p.t_step|}
  ...
  (* Tests for node_when and node_fork *)
  Printf.printf "Phase 3 — Static Conditionals (node_when, node_fork):\n";
  test "node_when true"
    {|p = pipeline {
  x = node_when(true, 1)
}
length(pipeline_nodes(p))|}
    "1";

  test "node_when false"
    {|p = pipeline {
  x = node_when(false, 1)
}
length(pipeline_nodes(p))|}
  ...
  test "node_fork first condition truthy"
    {|p = pipeline {
  x = node_fork(
    true, 1,
    false, 2
...
read_node(p.x)|}
  ...
  test "node_fork second condition truthy"
    {|p = pipeline {
  x = node_fork(
    false, 1,
    true, 2
...
read_node(p.x)|}
  ...
  test "node_fork .default fallback"
    {|p = pipeline {
  x = node_fork(
    false, 1,
    false, 2,
...
read_node(p.x)|}
  ...
  test "node_fork .default position independent"
    {|p = pipeline {
  x = node_fork(
    .default = 3,
    false, 1,
...
read_node(p.x)|}
  ...
  test "node_fork no match no default"
    {|p = pipeline {
  x = node_fork(
    false, 1
  )
...
length(pipeline_nodes(p))|}
  ...
  test "node_fork odd arguments error"
    {|p = pipeline {
  x = node_fork(
    false, 1,
    true
...
p|}
    "expects an even number of positional arguments";

  test "node_fork unexpected named argument error"
    {|p = pipeline {
  x = node_fork(
    false, 1,
    .unexpected = 42
...
p|}
    "received unexpected named argument '\\.unexpected'";
