let run_tests _pass_count _fail_count _eval_string _eval_string_env test =
  ...
  test "fill rejects invalid direction"
    {|df_fill = to_dataframe([[x: 1], [x: NA]]); fill(df_fill, $x, .direction = "sideways")|}
...
result = replace_na(df_replace, [i: 9, f: 9.5, s: "missing", b: false]); result.s|}
    {|Vector["x", "missing"]|};
  test "replace_na rejects non-to_dataframe input"
    {|replace_na(42, [x: 1])|}
...
select(wide, where(is_logical)) |> ncol|}
  ...
  test "separate splits column into multiple"
    {|df = to_dataframe([[x: "a-b-c"]]); separate(df, $x, into = ["v1", "v2", "v3"], sep = "-") |> colnames()|}
    {|["v1", "v2", "v3"]|};
  test "separate handles too few parts"
    {|df = to_dataframe([[x: "a-b"]]); separate(df, $x, into = ["v1", "v2", "v3"], sep = "-") |> pull("v3")|}
    "Vector[\"\"]";
  test "separate_rows expands rows by splitting"
    {|df = to_dataframe([[id: 1, x: "a;b"]]); separate_rows(df, $x, sep = ";") |> nrow|}
    "2";
  test "separate_rows preserves other columns"
    {|df = to_dataframe([[id: 1, x: "a;b"]]); separate_rows(df, $x, sep = ";").id|}
    "Vector[1, 1]";
  test "uncount duplicates rows by weight"
    {|df = to_dataframe([[x: "a", w: 3]]); uncount(df, $w) |> nrow|}
    "3";
  test "slice_max returns rows with largest values"
    {|df = to_dataframe([[x: 1], [x: 5], [x: 3]]); slice_max(df, $x, n = 2).x|}
    "Vector[5, 3]";
  test "slice_min returns rows with smallest values"
    {|df = to_dataframe([[x: 1], [x: 5], [x: 3]]); slice_min(df, $x, n = 2).x|}
    "Vector[1, 3]";
