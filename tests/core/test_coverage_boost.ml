let run_tests _pass_count _fail_count _failures _eval_string _eval_string_env test =
  ...
  test "case_when recycling"
    {|x = [1, 2, 3, 4]; case_when(x .< 2 ~ "small", x .< 4 ~ "medium", .default = "large")|}
    {|Vector["small", "medium", "medium", "large"]|};
  test "case_when no match returns NA"
    {|x = [1, 5]; case_when(x .< 2 ~ "small")|}
    {|Vector["small", NA]|};
  print_newline ();
  ...
  test "get from vector by index"
    {|df = to_dataframe([a: [10, 20]]); v = pull(df, "a"); get(v, 0)|}
    "10";
  test "get from to_dataframe by row lens"
    {|df = to_dataframe([mpg: [21, 22], cyl: [6, 6]]); get(df, row_lens(0)).mpg|}
    "21";
  test "get from to_dataframe by col lens"
    {|df = to_dataframe([mpg: [21, 22]]); get(df, col_lens("mpg")) |> head(n=1)|}
    "Vector[21]";
  test "compose lens"
    {|l = compose(row_lens(0), col_lens("mpg")); df = to_dataframe([mpg: [21, 22]]); get(df, l)|}
    "21";
  test "filter_lens on list"
    {|get([1, 10, 2, 20], filter_lens(\(x) x > 5))|}
    "[10, 20]";
  test "filter_lens on vector"
    {|df = to_dataframe([a: [1, 10, 2, 20]]); v = pull(df, "a"); get(v, filter_lens(\(x) x > 5))|}
    "Vector[10, 20]";
  ...
  test "pretty_print to_dataframe" {|df = to_dataframe([a: [1, 2], b: [3, 4]]); pretty_print(df)|} "NA";
  test "pretty_print summary"
    {|df = to_dataframe([a: [1, 2]]); s = [class: "summary", model_class: "lm", _tidy_df: df]; pretty_print(s)|}
    "NA";
  ...
  test "sum on vector" {|df = to_dataframe([a: [1, 2, 3]]); v = pull(df, "a"); sum(v)|} "6";
  test "sum on vector with NA" {|df = to_dataframe([a: [1, NA, 3]]); v = pull(df, "a"); sum(v, na_rm=true)|} "4";
  ...
  test "to_float on vector" {|df = to_dataframe([a: ["1.1", "2.2"]]); v = pull(df, "a"); to_float(v)|} "Vector[1.1, 2.2]";
  test "to_float on list" "to_float([1, \"2.2\"])" "[1., 2.2]";
