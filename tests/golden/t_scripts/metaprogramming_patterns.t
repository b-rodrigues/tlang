...
my_mutate = \(col: Any -> DataFrame) {
  col_enq = enquo(col)
  e = quo(iris |> mutate(!!col_enq := !!col_enq + 10.0))
  eval(e)
}
...
build_expr = \(x: Any -> Expr) quo(!!x + 5)
res = build_expr(quo(10 + 10))

res_df = eval(quo(iris
  |> summarize(val = !!res)
  |> head(1)))
...
write_csv(iris |> head(1) |> mutate(!!!multi_quo), path_join(output_dir, "metaprog_quos.csv"))
