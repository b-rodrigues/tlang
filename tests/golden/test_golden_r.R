...
test_that("ONNX: Random Forest predictions (iris)", {
  if (file.exists("tests/golden/data/iris_rf.onnx")) {
    compare_csvs("iris_onnx_rf_predictions")
  } else {
    skip("iris_rf.onnx not found")
  }
})
...
# nest_grouped_implicit: expected CSV is identical to mtcars_nest_grouped.csv
# (manually maintained, not generated by generate_expected.R) because nest()
# with no columns produces the same result as nest(data = c(mpg, hp)) after
# unnest+select+arrange.
test_that("TIDYR: nest grouped implicit columns", {
  compare_csvs("mtcars_nest_grouped_implicit")
})
...
# tidyr_slice_max: expected CSV is maintained manually (not generated by
# generate_expected.R) because T and R disagree on tie-breaking at the boundary.
test_that("TIDYR: slice_max", {
  compare_csvs("tidyr_slice_max")
})

# tidyr_nest_unnest: expected CSV is maintained manually (not generated by
# generate_expected.R) because T preserves original row order while R's nest
# reorders rows by non-nested columns.
