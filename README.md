...
3. **Load & Inspect**: Load the node's output to inspect it via `read_node("node_name")` in the REPL to verify results. For workflows outside T, the companion packages in `/home/runner/work/tlang/tlang/r-package/` and `/home/runner/work/tlang/tlang/py-package/` provide R and Python `read_node()` helpers that read built artifacts from `_pipeline/`, defaulting to `readRDS()` in R and pickle-based deserialization in Python.
