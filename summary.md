...
- Node-level transforms: `filter_node(p, predicate)`, `filter_nodes(p, predicate)`, `errored_nodes(p)`, `mutate_node(p, ..., where = na())`, `rename_node(p, old_name, new_name)`, `select_node(p, ...)`, `arrange_node(p, field, direction = "asc")`
