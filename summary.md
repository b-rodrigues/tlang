...
- Dogfooding: popcraft property tests exercise the colcraft/stats verbs (`mutate`, `arrange`, `filter`, `group_by`+`summarize`, `distinct`, `head`, `slice_sample`, `left_join`, `fill`, `mean`, `sd`) over generated DataFrames with injected NAs, guarding row-count algebra and `na_rm` invariants; the `math`, `strcraft`, `core`, and `chrono` packages are covered with arithmetic identities, string round-trips/idempotence, functional primitives, date bounds, and date round-trips.
