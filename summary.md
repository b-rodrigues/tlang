...
- Pipeline node read-back: `pipeline_node_options(pipeline, node)` returns a Dict of a node's fully resolved configuration (runtime, serializer/deserializer, noop, deps, depth, functions, include, env_vars, args, shell, shell_args, flake) after any `set_pipeline_global_options` merges; unknown nodes are a `TypeError`.
