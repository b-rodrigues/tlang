...
- **Dry Runs**: `dry_run = true` reports the sorted topological execution order and caches status as a DataFrame (via `--dry-run`) without running the build.
...
- Execution and artifacts: `populate_pipeline(p, build = false)`, `build_pipeline(p, targets = na(), force = na(), dry_run = false, max_jobs = na(), cache = na(), builders = na(), keep_env = na(), sandbox = na())`, `pipeline_run(p)`, `read_pipeline(p)`, `read_node(name, which_log = na())`, `pipeline_copy(...)`, `inspect_pipeline(p)`, `list_logs()`, `trace_nodes(p)`, `inspect_node(name)`, `rebuild_node(name)`, `suppress_warnings(node)`, `build_log_history(p, n = na())`, `node_diff(node_a, node_b, log_a = "latest", log_b = "latest")`, `debug_node(node)`, `build_log(p)`, `build_log_to_frame(log)`, `collect_errors(p)`, `error_summary(errors)`, `error_chain(err1, err2)`, `pipeline_to_drv(p)`, `pipeline_to_store(p)`, `set_nix_defaults(nix_options)`, `pipeline_cache_status(p)`, `export_artifacts(target, archive_path)`, `import_artifacts(target_or_archive, archive_path = na())`, `inspect_artifacts(archive_path)`
