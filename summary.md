...
- Pattern-based dynamic branching: `expand_pipeline(p)` expands `map_pattern(...)`, `cross_pattern(...)`, `slice_pattern(...)`, `head_pattern(...)`, `tail_pattern(...)`, and `sample_pattern(...)` nodes into N branch copies (one per element of the dependency); supports List, Vector, and DataFrame dependencies; non-T runtime nodes return an error.
