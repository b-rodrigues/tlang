...
- Global pipeline options: `set_pipeline_global_options(pipeline, functions = [:], include = [])` returns a new pipeline with runtime-wide defaults (function files and sandbox includes) merged into every node. Per-node `functions`/`include` are appended after global values. Original pipeline is unchanged.
