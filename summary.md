...
- Global pipeline options: `pipeline_options(functions = [:], include = [])` declares runtime-wide defaults (function files and sandbox includes) shared across all subsequent pipeline nodes. Per-node `functions`/`include` are appended after global values.
