...
- Aggregation helpers: `n()`, `n_distinct(x)`
- Descriptive statistics: `mean(x, na_rm = false, weight = NA)`, `median(x, na_rm = false, weight = NA)`, `min(x, na_rm = false)`, `max(x, na_rm = false)`, `range(x, na_rm = false)`, `var(x, na_rm = false, weight = NA)`, `sd(x, na_rm = false, weight = NA)`, `iqr(x, na_rm = false, weight = NA)`, `mad(x, constant = 1.4826)`, `fivenum(x, na_rm = false, weight = NA)`, `quantile(x, probs, na_rm = false, weight = NA)`, `skewness(x, na_rm = false, weight = NA)`, `kurtosis(x, na_rm = false, weight = NA)`, `trimmed_mean(x, trim = 0.1, na_rm = false, weight = NA)`, `winsorize(x, probs = [0.05, 0.95], na_rm = false, weight = NA)`, `cv(x, na_rm = false, weight = NA)`, `normalize(x)`, `standardize(x)`, `scale(x)`, `huber_loss(actual, predicted, delta = 1.0)`
- Relationship helpers and distributions: `cor(x, y, na_rm = false, weight = NA)`, `cov(x, y, na_rm = false, weight = NA)`, `pnorm(x, mean = 0, sd = 1)`, `pt(x, df)`, `pf(x, df1, df2)`, `pchisq(x, df)`
- Modeling: `lm(data, formula, weight = NA)`, `predict(data, model)`, `summary(model)`, `fit_stats(model)`, `add_diagnostics(data, model)`, `coef(model)`, `conf_int(model)`, `nobs(model)`, `df_residual(model)`, `sigma(model)`, `dispersion(model)`, `vcov(model)`, `compare(model1, model2)`, `residuals(model)`, `augment(data, model)`, `score(data, model)`, `anova(model_or_models)`, `wald_test(model, hypothesis)`
