...
let register env =
  Env.add "standardize" (make_builtin ~name:"standardize" 1 (fun args _ ->
    match args with
    | [x] ->
        (match numeric_values ~label:"standardize" ~na_rm:false x with
         | Error e -> e
         | Ok xs ->
             let n = List.length xs in
             if n < 2 then Error.value_error "Function `standardize` requires at least 2 values."
             else
