    ...
    (make_builtin ~name:"compare" ~variadic:true 0 (fun args _env ->
      ...
      | Ok valid_models ->
        if List.length valid_models = 0 then
          ...
          let rec extract_infos acc = function
            | [] -> Ok (List.rev acc)
            | (i, m) :: rest ->
                let name = match List.assoc_opt "name" m with
                  ...
                  | _ -> string_of_int (i + 1)
                in
                (match List.assoc_opt "_tidy_df" m with
                 | Some (VDataFrame df) -> extract_infos ((name, df) :: acc) rest
                 | _ -> Error (Error.type_error (Printf.sprintf "Model %s has no tidy coefficient table." name)))
          in
          (match extract_infos [] (List.mapi (fun i m -> (i, m)) valid_models) with
           | Error err -> err
           | Ok model_infos ->
