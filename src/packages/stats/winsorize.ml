  ...
  Env.add "winsorize" (make_builtin ~name:"winsorize" 2 (fun args _ ->
    ...
    | [x; limits] ->
        (match numeric_values ~label:"winsorize" ~na_rm:false x, limits_of limits with
         ...
         | Ok [], _ -> VNA NAFloat
         | Ok xs, Some (lo, hi) ->
             let lq = match quantile xs lo with Some v -> v | None -> 0.0 (* unreachable: xs non-empty *) in
             let uq = match quantile xs (1.0 -. hi) with Some v -> v | None -> 0.0 (* unreachable: xs non-empty *) in
