...
let starts_with_impl args _env =
  match args with
...
let register env =
  let env = Env.add "starts_with" (make_builtin ~name:"starts_with" 1 starts_with_impl) env in
  let env = Env.add "ends_with" (make_builtin ~name:"ends_with" 1 ends_with_impl) env in
  let env = Env.add "contains" (make_builtin ~name:"contains" 1 contains_impl) env in
