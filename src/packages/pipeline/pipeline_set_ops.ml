...
let filter_node_set keep_set p =
  VPipeline {
    ...
    p_provenance   = List.filter (fun (n, _) -> keep_set n) p.p_provenance;
...
let union p1 p2 =
  ...
  if collisions <> [] then
    ...
    Ok (VPipeline {
      ...
      p_provenance   = p1.p_provenance @ p2.p_provenance;
...
let patch p1 p2 =
  ...
  VPipeline {
    ...
    p_provenance   = p1_filtered.p_provenance @ p2_filtered.p_provenance;
