  ...
  | latest_log :: _ ->
      ...
      | Ok entries ->
          ...
          let apply_perms path =
             let dcmd = Printf.sprintf "find %s -type d -exec chmod %s {} +" (Filename.quote path) dir_mode in
             let fcmd = Printf.sprintf "find %s -type f -exec chmod %s {} +" (Filename.quote path) file_mode in
